...
for (let n = 0; n < inventory.length; n++){
    let thisCar = inventory[n];
    if (thisCar.id == 33){
        console.log(`Car 33 is a ${thisCar.car_year} ${thisCar.car_make} ${thisCar.car_model}` );
    }

    //if car is not found with ID, nothing will be printed
}
...
let makeModel = `${inventory[lastCar].car_make} ${inventory[lastCar].car_model}`;
...
let carModels = [];
for (let n = 0; n < inventory.length; n++){
    carModels.push(inventory[n].car_model);
}
carModels.sort();
...
let carYears = [];
for (let n = 0; n < inventory.length; n++){
    carYears.push(inventory[n].car_year);
}
carYears.sort();
...
let oldCars = [];
for (let n = 0; n < inventory.length; n++){
    if (inventory[n].car_year < 2000){
        oldCars.push(inventory[n].car_year);
    }
}
...
let BMWAndAudi = [];
for (let n = 0; n < inventory.length; n++){
    if (inventory[n].car_make == "BMW" || inventory[n].car_make == "Audi"){
        BMWAndAudi.push(inventory[n]);
    }
}
